...
\begin{abstract}
Does a quantum-native signal representation, given every function of its classical peer, learn or reconstruct anything the classical one does not? We answer this for one representation with an audit, and the audit is the contribution. The vehicle is a quantum adaptive chirplet transform (\qact), whose atoms are exact shallow circuits. The audit has three parts: functional parity with a strong classical engine, pre-registered paired comparisons on real EEG, and a control that isolates each apparent win. Every apparent \qact\ advantage had a classical explanation: the transform's free 0.5\,Hz frequency axis, and a classical seed band that excludes mains interference. At equal function set the classical engine is at least as good. The quantum sampler dequantises exactly to a periodogram, computable in $O(N\log N)$. Two things survive: the dictionary is gate-native and needs no QRAM, and the matching-pursuit residual can be kept coherent by a one-ancilla reflection whose success probability equals the residual energy fraction, which we verify on circuits. On an IBM Heron processor, six-qubit selection circuits pick the exact atom in \qpuSixPeakOK\ of \qpuSixN\ cases and never fall below \qpuSixSelMin\ of the best atom's energy; seven-qubit circuits are marginal and long atoms fail. Downstream, the chirplet features themselves add 9--13 accuracy points over amplitude features for cross-patient seizure detection on two databases, while quantum kernels, classifiers, recurrent models and reservoirs land at parity or below. A fully controlled parity result is the baseline any quantum advantage for recorded signals has to clear.
\end{abstract}

\section{Introduction}
Quantum methods for recorded signals are usually judged by whether they run and whether they win. We ask the question a reviewer of any new representation would ask: \emph{given every function of its classical peer, does a quantum-native representation learn or reconstruct anything the classical one does not, and if not, why not?} We answer it for one representation with a three-part audit: \emph{functional parity} with a strong classical engine, so that the two are compared as equals; \emph{pre-registered comparisons} on real EEG, with paired tests across subjects or patients; and, for each apparent win, a \emph{control that isolates its cause}. The audit is the contribution; the transform is the vehicle.

The vehicle is the adaptive chirplet transform (\act) of \citet{mann1991chirplet,mann1992adaptive,mann1995chirplet}. It writes a signal as a few Gaussian-windowed linear chirps chosen greedily by matching pursuit \citep{mallat1993matching,bultan1999four}. The chirp rate, which a stationary spectrum cannot report, makes it informative for drifting EEG transients. The quantum version rests on one observation: a chirp's quadratic phase is exactly 2-local in the binary encoding of time, so an atom is an $O(n^2)$-gate circuit on $n=\log_2N$ qubits. One inverse quantum Fourier transform (QFT) then returns an atom's whole frequency axis, measurement samples atoms with the probability matching pursuit maximises, and the parameter-shift rule refines every phase parameter exactly (\S\ref{sec:qact}). Earlier quantum matching-pursuit proposals assume quantum random-access memory (QRAM) for signal and dictionary, and have not been run on data \citep{bellante2022qmp,bellante2025qomp}.

\paragraph{The obvious objection.} A reader's first objection is that this sampler is just a periodogram. It is. For each envelope and chirp rate, the sampled distribution is the squared magnitude of one FFT, computable classically in $O(N\log N)$, and a classical argmax over it is the transform with infinitely many shots (Proposition~\ref{prop:dequant}). Two pieces of quantum structure survive. The dictionary is gate-native, so a superposition over atoms costs controlled gates, not a QRAM holding the dictionary. And the matching-pursuit residual can be kept coherent, with success probability equal to the fraction of energy still unexplained (Proposition~\ref{prop:coherent}). Neither is a speedup at EEG sizes (\S\ref{sec:theory}). Why audit it, then? Because apparent quantum wins on real data are easy to produce and hard to dissolve: this transform produced three apparent wins, each with a classical cause that only a targeted control revealed. And a representation is where a classical baseline can be matched exactly, since the classical peer is a program, not a model class. A controlled parity result there is the bar a claimed advantage has to clear.

\paragraph{What the controls found.} \qact\ first appeared to win on denoising against ICA-cleaned ground truth (17 of 18 held-out subjects, $p<10^{-4}$). No bug produced the win. Its cause was the transform's frequency axis (every 0.5\,Hz QFT bin from 0.5 to 45\,Hz, against the shipped classical grid's 1\,Hz steps over 1--40\,Hz), which the classical engine can copy. A hardware-motivated refiner then appeared to beat the frequency-matched classical engine. Giving the classical engine the same refiner does \emph{not} close that gap; extending its seed grid past 45\,Hz, to the 50\,Hz mains in the injected artifacts, does. On public data the shipped grid failed on windows dominated by 60\,Hz mains, so \qact\ looked better in the mean but not the median. Each effect is a legitimate engineering finding; none is a quantum one. Learning has one positive result (Table~\ref{tab:downstream}): chirplet features add 9--13 accuracy points over amplitude features for cross-patient seizure detection, on two databases and two classifiers. Swapping in \qact\ does not raise that number, and quantum kernels, variational classifiers, quantum recurrent models and quantum reservoirs land at parity or below.

\paragraph{Contributions.}
(1) A pre-registered audit on real EEG (\S\ref{sec:exp}): denoising with ground truth on 26 subjects, seizure detection on 37 patients from two databases, and per-person forecasting on 983 hours, with the controls that dissolved each apparent advantage (three run for this paper).
(2) A quantum \act\ with exact circuits for the atom, the selection step and the refinement gradient, at full functional parity with a strong classical engine (\S\ref{sec:qact}); the post-selection-conditioned distribution a device reports \emph{is} the classical normalised criterion, which fixed a 70\% deficit.
(3) The exact dequantisation of the sampler, and what survives it (\S\ref{sec:theory}): a gate-native dictionary with no QRAM, and a coherent residual with success probability equal to the residual energy fraction, verified on circuits to $10^{-9}$.
(4) The first execution of the transform's selection step on a quantum processor (\S\ref{sec:hw}): a short atom cut from 2,027 to about 140 CZ gates, selecting the exact atom at six qubits, degrading at seven, and failing for long atoms.
...
\label{eq:atom}
\end{equation}
with $f_c$ in cycles per window and $c$ in cycles per window per sample. Matching pursuit picks the atom maximising $|\braket{\psi_\theta}{r}|^2$ against the residual $r$, refines $\theta$ off-grid, subtracts, and repeats; orthogonal matching pursuit (OMP) re-fits all coefficients jointly \citep{pati1993omp}. The classical engine used throughout is a batched GPU implementation of the \act\ \citep{actcode}; residual-projection matching pursuit and hybrid GPU acceleration for the \act\ are developed by \citet{kumar2026stable}. The engine selects by the least-squares energy of the cosine/sine quadrature pair, refines with sixty steps of Adam on the same objective, re-fits by OMP, and stops per window by rule. A two-width asymmetric envelope \citep{actcode} is available as a variant.

\paragraph{Seed grids.} All classical seed grids have 17 centres, 6 widths and 5 chirp rates and differ only in frequency; we name the grid wherever it matters. The \emph{shipped grid} (the engine as released, used unless another is named) has 40 frequencies at 1\,Hz, 1--40\,Hz: $17\times40\times6\times5=20{,}400$ atoms. The \emph{EEGMAT 0.5\,Hz grid} (EEGMAT grid for short), used for denoising, matches \qact's band and spacing (every QFT bin of a 512-sample window at 256\,Hz): 90 frequencies, 0.5--45\,Hz, $17\times90\times6\times5=45{,}900$ atoms. The \emph{public 0.5\,Hz grid}, used on the public motor-imagery windows, keeps the shipped band at 0.5\,Hz spacing (80 frequencies, 1--40.5\,Hz, 40,800 atoms). The seed-band controls extend the EEGMAT grid (62,730 atoms) or the shipped grid (39,780 atoms) to 78\,Hz in 1\,Hz steps.
...
Expanding $c(t-t_c)^2+f_c(t-t_c)=c\,t^2+(f_c-2ct_c)\,t+\text{const}$ shows that a shifted chirp is the same quadratic phase plus a linear phase at the \emph{effective frequency} $f=f_c-2ct_c$; the centre time enters only through the envelope. With $t=\sum_k b_k2^k$ and $b_k^2=b_k$,
\begin{equation}
...
\label{eq:twolocal}
\end{equation}
a product of $n$ phase gates and $n(n-1)/2$ controlled-phase gates, exactly.\footnote{Verified against the analytic diagonal and against a statevector simulation \citep{qiskit2024} to $9\times10^{-13}$ and $4\times10^{-14}$.} The linear phase is $n$ single-qubit gates. A cubic phase, an atom whose chirp rate itself evolves, is 3-local by the same argument (Table~\ref{tab:parity}). The Gaussian envelope is a real diagonal filter, realised either by a multiplexed rotation onto an ancilla with post-selection or, as we do on hardware, by folding it into state preparation (\S\ref{sec:hw}).
...
\label{sec:parity}
A transform that only selects atoms is not the classical engine. Table~\ref{tab:parity} lists every function of the classical engine and its quantum-native counterpart. Each is opt-in, and with all of them off the transform reproduces the pre-parity reference feature matrix bit for bit (2.5 million entries). The OMP refit costs no circuits: its normal matrix is the Gram matrix of atom overlaps $\braket{\psi_i}{\psi_j}$, a swap-test observable in principle, but it depends only on atom parameters, so it and the right-hand side are classical. The exact frequency update follows from eq.~\eqref{eq:sampler}: $E(f)$ is the periodogram, so the optimal $f$ is read off one transform instead of gradient-stepped toward. Two functions have no counterpart in the classical engine we compare against (the exact frequency update and backfit), so the control study of \S\ref{sec:control} adds backfit to the classical engine as well.
...
\end{proposition}
The proof is the definition of the QFT (Appendix~\ref{app:proofs}). The consequence is that an \emph{enumerated} \qact\ with $S$ shots per triple costs $\Theta(M'\,S\,N)$ gate-time per iteration (state preparation of an arbitrary $N$-vector needs $\Theta(N)$ gates without QRAM \citep{mottonen2005transformation,shende2006synthesis}) against $\Theta(M'N\log N)$ classical operations. A classical program that computes eq.~\eqref{eq:sampler} by FFT and takes the argmax is exactly the transform with infinitely many shots. We ran that program. On the GPU it decomposes a window in \speedQSimBatched\ (batched), faster than the classical engine (\speedClGPUBatched) because it uses four cheap refinement steps instead of sixty, at a worse fit (\speedQSimErr\ versus \speedClGPUErr). It is a classical algorithm that computes what the circuit would.

Two pieces of quantum structure are not dequantised by Proposition~\ref{prop:dequant}. The first is that the dictionary is \emph{gate-native}: an atom is $O(n^2)$ gates plus a Gaussian preparation \citep{grover2002creating}. A superposition over the $M'$ triples in index registers therefore costs controlled versions of those gates, not a QRAM of $M'N$ amplitudes as quantum matching pursuit assumes \citep{bellante2022qmp,jaques2025qram}. Quantum maximum finding over the index registers then selects the best triple in $O(\sqrt{M'})$ evaluations \citep{durr1996minimum,brassard2002amplitude}. The second is the residual.
...
The coherent residual removes the classical computation of $r_k$ between iterations, not the $\Theta(N)$ load of $x$, which every Grover repetition repeats. Without QRAM the superposed variant therefore costs $O\big(\sqrt{M'}\,\varepsilon^{-1}(N+Kn^2)\big)$ gate-time per iteration for amplitude resolution $\varepsilon$, against $O(M'N\log N)$ classically. The gain is polynomial in the dictionary size only, and the clock-speed and shot costs of \S\ref{sec:hw} eat it at the dictionary sizes EEG needs ($M'\le 5{,}000$). The dequantisation literature \citep{aaronson2015fineprint,tang2019quantum,chia2020sampling,tang2022dequantizing} predicts exactly this split (the QRAM regime is in Appendix~\ref{app:proofs}), and the experiments below are consistent with it.


\section{Experiments on real EEG}
\label{sec:exp}
\paragraph{Protocol.} Every comparison was pre-registered with its decision rule before it was run: subject-wise or patient-wise folds, paired Wilcoxon signed-rank tests across subjects or patients, and Bonferroni correction over the tests run on a dataset. The two frequency-matched denoising arms were added after the parity result was seen, and were named as the deciding controls before they ran. Negative results are reported in place. Seed grids are named as in \S\ref{sec:background}. Datasets: EEGMAT \citep{zyma2019eegmat}, 36 subjects, ICA-cleaned by its authors, 500\,Hz resampled to 256\,Hz; CHB-MIT \citep{shoeb2009application}, 23 cases of 22 paediatric patients, 198 seizures, 983\,h; the Siena scalp EEG database \citep{detti2020siena}, 14 patients, 45 seizures, used once as a confirmation set; and, for the controls added in this paper, the PhysioNet EEG motor movement/imagery baseline runs \citep{schalk2004bci2000,goldberger2000physionet}, 20 subjects, read with MNE \citep{gramfort2013mne}. All data are public \citep{goldberger2000physionet}.
...
\begin{table}[t]
\caption{EEGMAT denoising, 18 held-out subjects: score (mean over subjects, lower is better; time fidelity plus per-band dB error), relative RMSE and delta-band error (medians over subjects), and the paired comparison (subjects better, of 18; Wilcoxon $p$). Grids as in \S\ref{sec:background}; ``vs grid'' is against the classical EEGMAT grid (1.257), ``vs hw'' against \qact\ with the hardware refiner (\denoiseRefQ). Bonferroni $\alpha=0.0056$ (nine tests) for the first block, $0.0125$ (four) for the second and for the two arms run for this paper; the hardware-refiner test clears both.}
\label{tab:denoise}
\centering\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lrrrl@{}}
...
\midrule
no cleaning & 3.487 & -- & -- & \\
classical engine (shipped grid) & 1.439 & 0.428 & 0.73 & reference \\
...
classical, shipped grid + asymmetric envelope & 1.375 & 0.417 & 0.71 & \\
classical, EEGMAT 0.5\,Hz grid (45,900 atoms) & 1.257 & 0.423 & 0.62 & \\
classical, EEGMAT grid + asymmetric envelope & 1.160 & 0.408 & 0.60 & \\
\midrule
\multicolumn{5}{@{}l}{\emph{the remaining gap: \qact's hardware refiner, and two controls run for this paper}} \\
\qact\ at parity + hardware coordinate refiner & \denoiseRefQ & 0.392 & 0.57 & vs grid: better on 14/18, $p=0.0007$ \\
\ifdefined\denoiseArmsPresent
classical, EEGMAT grid + coordinate refiner & \denoiseArmAScore & 0.422 & 0.61 & vs hw: better on \denoiseArmAVsQBetter/18, $p=\denoiseArmAVsQP$ \\
 & & & & vs grid: better on \denoiseArmAVsCBetter/18, $p=\denoiseArmAVsCP$ \\
classical, EEGMAT grid extended to 78\,Hz & \denoiseArmBScore & 0.380 & 0.61 & vs hw: better on \denoiseArmBVsQBetter/18, $p=\denoiseArmBVsQP$: no difference \\
 & & & & vs grid: better on \denoiseArmBVsCBetter/18, $p=\denoiseArmBVsCP$ \\
\fi
\midrule
\qact\ at parity vs classical, EEGMAT grid & & & & better on 6/18, $p=0.15$: no difference \\
\qact\ restricted vs classical, EEGMAT grid & & & & better on 4/18, $p=0.008$: classical better \\
\qact\ parity + asym vs classical, EEGMAT + asym & & & & better on 11/18, $p=1.00$: no difference \\
\bottomrule
\end{tabular}}
\end{table}

The reading changed twice. Before parity, \qact\ was significantly worse on every subject. The discriminating band was delta: its slow atoms matched the waveform less precisely, and subtracting them removed real EEG. After parity, every \qact\ arm beat the classical engine on 17 or 18 of 18 subjects. Two controls left that standing: holding \qact\ to the classical function set, and giving the classical engine its own asymmetric envelope. The third dismantled it. \qact's dictionary covers every QFT bin from 0.5 to 45\,Hz, because one transform returns the whole axis; the shipped grid steps at 1\,Hz over 1--40\,Hz. On the EEGMAT 0.5\,Hz grid, which has \emph{more} atoms than \qact\ (45,900 versus 22,784), the classical engine scores 1.257, and the paired test against \qact\ at parity finds no difference ($p=0.15$). The arm holding \qact\ to the classical function set is then significantly worse. The frequency axis is a real structural property of the transform, worth more than any engine function measured here.\footnote{The EEGMAT grid differs from the shipped grid in band as well as spacing, so this gain combines the two; no arm separates them, and neither is quantum. On the public windows, finer spacing alone changes nothing (\S\ref{sec:control}).} It is also a constant factor, which the classical engine buys with a 2.25$\times$ larger dictionary.

One gap remained. With the hardware refiner of \S\ref{sec:hw} (three-point coordinate search), \qact\ scores \denoiseRefQ, better than the classical engine on the EEGMAT grid (1.257) on 14 of 18 subjects ($p=0.0007$). The obvious explanation, the refinement algorithm, is wrong. Given the same coordinate search, the classical engine scores \denoiseArmAScore, \emph{worse} than \qact\ with the hardware refiner on 17 of 18 subjects ($p=\denoiseArmAVsQP$). Given the EEGMAT grid extended from 45 to 78\,Hz, so that it can seed the 50\,Hz mains the artifact injection contains, it scores \denoiseArmBScore. That is indistinguishable from \qact\ (better on \denoiseArmBVsQBetter/18, $p=\denoiseArmBVsQP$), and better than the unextended EEGMAT grid on \denoiseArmBVsCBetter\ of 18 subjects ($p=\denoiseArmBVsCP$). \qact's exact frequency update reads the periodogram peak wherever it is; from the EEGMAT grid's top seed at 45\,Hz, sixty Adam steps reach only 45--47\,Hz. The seed band, not the refiner, is the explanation, and every denoising advantage of the transform now has one.

\subsection{The refinement and dictionary controls}
\label{sec:control}
The denoising controls isolate the seed band on one dataset and one task. To separate refiner, dictionary and band on public data, with no artifact rule in the way, we run the same factors on 320 public windows (20 subjects, two baseline runs, eight channels, 512 samples at 160\,Hz, zero-mean, unit norm). The score is the relative residual norm after OMP at 6 and 12 atoms (Table~\ref{tab:fair}; error against time is plotted in Fig.~\ref{fig:fair}, Appendix~\ref{app:fair}). The engines are the classical engine as shipped (shipped grid, Adam, 60 steps), with \qact's budget (Adam, 4 steps) and with \qact's coordinate rule (4 sweeps, step sizes in physical units); the public 0.5\,Hz grid with Adam and with the coordinate rule; and \qact\ with parameter shift, with the coordinate rule, and in its full parity configuration (coordinate rule, exact frequency update, one backfit pass).
...
\midrule
classical, shipped grid, Adam 60 steps (reference) & \fairClAdamErrSix & -- & \fairClAdamErrTwelve & -- & \fairClAdamMsTwelve \\
classical, shipped grid, Adam 4 steps & \fairClAdamFourErrSix & \fairClAdamFourRelSix\%$^{\ast}$ & \fairClAdamFourErrTwelve & \fairClAdamFourRelTwelve\%$^{\ast}$ & \fairClAdamFourMsTwelve \\
classical, shipped grid, coordinate search, 4 sweeps & \fairClCoordErrSix & \fairClCoordRelSix\%$^{\ast}$ & \fairClCoordErrTwelve & \fairClCoordRelTwelve\%$^{\ast}$ & \fairClCoordMsTwelve \\
classical, public 0.5\,Hz grid, Adam 60 & \fairClFineAdamErrSix & \fairClFineAdamRelSix\% ($p=\fairClFineAdamPSix$) & \fairClFineAdamErrTwelve & \fairClFineAdamRelTwelve\% ($p=\fairClFineAdamPTwelve$) & \fairClFineAdamMsTwelve \\
classical, public 0.5\,Hz grid, coordinate 4 & \fairClFineCoordErrSix & \fairClFineCoordRelSix\%$^{\ast}$ & \fairClFineCoordErrTwelve & \fairClFineCoordRelTwelve\%$^{\ast}$ & \fairClFineCoordMsTwelve \\
...
Three things follow. First, at equal refinement rule the classical engine is better than \qact. With the same coordinate search it is \fairCoordVsHwRelTwelve\% below \qact\ at order 12, better on \fairCoordVsHwBetterTwelve\ of \fairN\ windows ($p=\fairCoordVsHwPTwelve$). The coordinate search itself costs the classical engine \fairClCoordRelTwelve\% against sixty Adam steps, at a tenth of the time, consistent with the denoising arm above. Second, finer spacing alone (the public 0.5\,Hz grid, same band as the shipped grid) changes nothing on this metric at these orders (\fairClFineAdamRelTwelve\% at order 12, $p=\fairClFineAdamPTwelve$). The EEGMAT grid's denoising gain in \S\ref{sec:denoise}, which combines finer spacing with a wider band, is a property of the denoising task, whose mechanism we did not isolate. Third, \qact\ at parity is \fairQParityRelTwelve\% \emph{better} than the reference in the mean at order 12, but better on only \fairQParityBetterTwelve\ of \fairN\ windows, with a median difference of $\fairParityMedianDiffTwelve$. The mean is carried by \fairRefFailTwelve\ windows on which the classical engine leaves more than 90\% of the energy. Those windows are dominated by 60\,Hz mains interference (\fairDriftFailTwelve\% of their energy above 45\,Hz, against \fairDriftOKTwelve\% for the rest). The shipped grid stops at 40\,Hz and cannot seed it; \qact's exact frequency update reads the periodogram peak wherever it is. Extending the shipped grid to 78\,Hz leaves \fairLowFailTwelve\ such windows, and the classical engine is then \fairLowVsParityRelTwelve\% relative to \qact\ parity, better on \fairLowVsParityBetterTwelve\ of \fairN\ ($p=\fairLowVsParityPTwelve$; with backfit added, Appendix~\ref{app:fair}). Every remaining difference is a band, dictionary or refiner choice the classical engine can make.
...
The chirplet features are the one solid positive result. They add 9--13 accuracy points beyond amplitude features for cross-patient seizure detection, significant on two databases and two classifiers (absolute accuracies in Appendix~\ref{app:downstream}). The quantum transform does not improve that number. Under univariate top-$k$ selection \qact\ features led by 1.2 to 3.0 points, never significantly; under all features with regularisation the classical features led. With the parity engine, measurement-sampled \qact\ is 4.5 to 7.2 points behind on CHB-MIT and 1.8 to 5.3 behind on Siena, because sampling atoms by measurement costs 4--5 points against the argmax on the same engine. Atom-level improvements that are real produced no downstream gain: reconstruction quality and classification value were close to unrelated, three times over (Appendix~\ref{app:downstream}). Among the quantum learners, one effect survived its own control, and only at fine time resolution. At 2\,s steps entanglement contributes to the quantum reservoir's forecasting skill for all 23 people; at 60\,s steps it does not; and the reservoir loses to a classical echo-state network of the same readout size at every resolution.

\section{On hardware}
\label{sec:hw}
On a device the bill is circuits $\times$ shots $\times$ two-qubit gates, and the construction as drafted was expensive on all three. The envelope as a $2^n$-way multiplexed rotation with post-selection discarded 43--97\% of shots on real EEG, and a nine-qubit circuit transpiled to 2,027 CZ gates on a heavy-hex IBM Heron target. Three changes, each exact, reduce it (Fig.~\ref{fig:hardware}). \emph{Folding} the envelope into state preparation loads $e\odot r$ directly: no ancilla, no multiplexor, every shot kept (\hwFoldedCZ\ CZ). The \emph{semiclassical} inverse QFT of \citet{griffiths1996semiclassical} measures each qubit as soon as it is final and replaces every controlled phase by a classically controlled single-qubit phase, so the QFT contains no two-qubit gates (\hwSemiclassicalCZ\ CZ; a dynamic circuit). \emph{Window-local registers} use shift covariance, $c(u+t_0)^2=cu^2+2ct_0u+\text{const}$: a Gaussian atom needs a register only as large as its $\pm4\sigma$ support, the offset folds into one linear phase, and the smaller QFT returns the original distribution exactly on every $2^{n-m}$-th bin. A short atom then needs 6 qubits and about 140 CZ gates. Under the Heron noise model its selection survives (selected-atom energy \hwWindowedSel\ on average over widths, 0.99--1.00 for the two short widths) where the baseline never did (\hwBaselineSel). Long atoms ($\log\Delta t\ge3.9$) span the window, cannot be shrunk, and lose their selection under noise; what remains for them is state preparation, about 87\% of the two-qubit budget \citep{holmes2020efficient,moosa2023linear}. Branch-and-bound over envelopes and the three-point refiner cut circuit executions per atom about 80-fold (Appendix~\ref{app:hw}).
...
\paragraph{Real device.} We executed the windowed construction on \texttt{\qpuBackend}, a 156-qubit Heron processor, for \qpuN\ circuits: six public EEG windows (three subjects, two channels) at two short widths, each in the semiclassical (dynamic) form and in a unitary-QFT form that admits gate twirling, plus one long-atom case in both forms. Each ran \qpuShots\ shots with dynamical decoupling (jobs \texttt{\qpuJobs}, \qpuSeconds\ s of processor time). The classical certificate for every circuit is the exact distribution of Proposition~\ref{prop:dequant}. The result splits by register size. The \qpuSixN\ six-qubit circuits (\qpuSixCZ\ CZ on average) selected the exact in-band atom in \qpuSixPeakOK\ cases, and the atom they did select carried \qpuSixSel\ of the best atom's energy on average, never less than \qpuSixSelMin: selection survives (Fig.~\ref{fig:device}). The \qpuSevenN\ seven-qubit circuits (\qpuSevenCZ\ CZ) are marginal, with the exact atom in \qpuSevenPeakOK\ cases, a mean of \qpuSevenSel, and \qpuSevenLost\ outright losses. The two nine-qubit long-atom circuits (\qpuLongCZ\ CZ) reached fidelity \qpuLongFid\ and selected-atom energy \qpuLongSel: selection is lost, as the noise model predicted. The calibrated noise model is optimistic on this device by about 0.15--0.2 in fidelity (Appendix~\ref{app:hw}).

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{fig_device.pdf}
...
\paragraph{Speed.} Priced on the same job (four EEGMAT windows, six atoms, OMP; Table~\ref{tab:speed}, Appendix~\ref{app:hw}), the optimised circuits would take \speedHeron\ per window on a Heron processor, and \speedAllThree\ with three published throughput techniques. That is still about 140$\times$ slower than the classical engine on a CPU and 12,000$\times$ slower than the batched GPU engine. Per shot, the 250\,$\mu$s repetition delay dwarfs the 16--96\,$\mu$s circuit, and each circuit needs thousands of shots to estimate what an FFT computes exactly in microseconds.


\section{Discussion}
\paragraph{What survives.} Three things in \qact\ are genuinely quantum-native, and none is an advantage. The frequency axis is free, a constant factor the classical engine buys with a larger grid. The dictionary is gate-native, which removes the dictionary half of the QRAM assumption; the signal half remains, and the coherent residual costs a success probability equal to the unexplained energy. And the shortest atoms select correctly on today's hardware, a statement about circuit design under noise, not speed. Every other apparent win had a classical explanation found by a control; one (17 of 18 subjects, $p<10^{-4}$) fell only to the third control. This is the matched-baseline discipline \citet{bowles2024better} and \citet{schuld2022quantum} ask of quantum learning, applied where it is easiest to satisfy.

\paragraph{A search that ended negative.} After parity, the study searched for any quantum chirplet transform that beats the strongest classical baseline, including a classical port of the same idea, in speed at equal accuracy or accuracy at equal time, a criterion fixed in advance. None did. Hybrid routing, error correction, a powered 60-window test, fault-tolerant coherent search, kernels on atoms, small-data regimes and entangled sensing all land at parity or below, or cross over only at unreachable scales (Appendix~\ref{app:search}).

\paragraph{Why no advantage should be expected here.} Recorded signals must be loaded into every circuit for every shot, which is Aaronson's fine print \citep{aaronson2015fineprint}. Once the classical side gets equivalent access, the advantage is at most polynomial \citep{tang2019quantum,chia2020sampling}, and Proposition~\ref{prop:dequant} makes this exact for the transform. The published exponential advantages for signals concern signals that reach a qubit directly \citep{huang2022advantage,kannan2026exponential}; an EEG amplifier does not.

\paragraph{Limitations.} The audit covers one transform family, three EEG databases and one device. The hardware runs demonstrate selection for individual circuits, not a full decomposition on a QPU. The coherent-residual construction is verified at $n=6$ with an unoptimised Gaussian preparation. The learners are those a practitioner would try. Nothing here rules out an advantage for a quantum-native signal.

\section{Conclusion}
For the adaptive chirplet transform, the answer to our question is no: given every function of its classical peer, the quantum-native representation learned or reconstructed nothing the classical one did not. It can be built exactly, refined exactly, brought to full functional parity with a strong classical engine, and run for the shortest atoms on a quantum processor. Under pre-registered comparisons with paired tests on real EEG it is its classical peer's equal and beats nothing; each apparent advantage was a dictionary or seed-band choice the classical engine can copy, and three of the controls that show it are new here. A fully controlled parity, with the controls named before they ran, is the baseline a claimed quantum advantage for recorded signals has to clear.
...
\paragraph{Complexity of the superposed search.} With the triples $(t_c,\Delta t,c)$ in index registers, the envelope preparation and the phase gates of eq.~\eqref{eq:twolocal} become controlled on those registers; the outcome register holds $k$. Quantum maximum finding \citep{durr1996minimum} over $(\text{index},k)$ with the amplitude estimated to precision $\varepsilon$ \citep{brassard2002amplitude} costs $O(\sqrt{M'N_f}\,\varepsilon^{-1})$ preparations of the searched state, each of which loads $x$ ($\Theta(N)$ gates without QRAM) and applies the $k-1$ reflections of Proposition~\ref{prop:coherent} ($O(kn^2)$ plus envelope preparations), giving the bound quoted in \S\ref{sec:theory}. The $N_f$ factor is absorbed into $M'$ in the text. With coherent access to the signal (QRAM, or a signal that reaches a qubit directly, \citealp{huang2022advantage,kannan2026exponential}), the same construction runs in $O(\sqrt{M'}\varepsilon^{-1}(Kn^2+\mathrm{polylog}\,N))$, which is the regime the quantum matching-pursuit literature analyses.

\section{Refinement control: additional detail}
\label{app:fair}
Table~\ref{tab:fairbf} gives the backfit and extended-band arms; the extended arm is the shipped grid extended to 78\,Hz (39,780 atoms, \S\ref{sec:background}). Windows on which the classical engine as shipped leaves more than 90\% of the energy have \fairDriftFailTwelve\% of their energy above 45\,Hz (mains); the others \fairDriftOKTwelve\%. With the backfit pass \qact\ parity has, the classical engine on the extended grid is \fairLowBfVsParityRelTwelve\% relative to \qact\ parity at order 12 (better on \fairLowBfVsParityBetterTwelve\ of \fairN, $p=\fairLowBfVsParityPTwelve$).

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{fig_fair.pdf}
\caption{Reconstruction error against GPU time per window for the engines of Table~\ref{tab:fair} (grey: classical; blue: \qact). At equal refinement rule the classical engine is ahead; the \qact\ parity mean at order 12 is carried by the windows on which the shipped grid fails (\S\ref{sec:control}).}
\label{fig:fair}
\end{figure}
...
\midrule
classical, shipped grid, Adam 60 (reference) & \fairBfRefErrSix & \fairBfRefErrTwelve & \fairBfRefMedTwelve & -- & \\
classical, shipped grid + backfit 1 & \fairBfOneErrSix & \fairBfOneErrTwelve & \fairBfOneMedTwelve & \fairBfOneRelTwelve\% & \fairBfOneBetterTwelve\ / $\fairBfOnePTwelve$ \\
classical, shipped grid to 78\,Hz & \fairBfLowErrSix & \fairBfLowErrTwelve & \fairBfLowMedTwelve & \fairBfLowRelTwelve\% & \fairBfLowBetterTwelve\ / $\fairBfLowPTwelve$ \\
classical, shipped grid to 78\,Hz + backfit 1 & \fairBfLowOneErrSix & \fairBfLowOneErrTwelve & \fairBfLowOneMedTwelve & \fairBfLowOneRelTwelve\% & \fairBfLowOneBetterTwelve\ / $\fairBfLowOnePTwelve$ \\
...
\paragraph{Seizure detection.} CHB-MIT: 10,856 two-second epochs (5,501 ictal / 5,355 interictal), 23 patients, cross-patient folds. With all 270 chirplet + amplitude features and regularisation (no selection), the gain of chirplet features over amplitude-only, paired across patients (decision threshold $p<0.0036$): logistic regression $C=0.01$, raw, $+8.6$ points ($p=0.0011$, 20/23); $C=0.1$, raw, $+7.5$ ($p=0.0005$, 20/23); RBF SVM, raw, $+3.4$ ($p=0.050$); logistic $C=0.01$, per-patient normalised, $+3.5$ ($p=0.0096$); $C=0.1$, normalised, $+3.4$ ($p=0.0015$, 17/23); RBF SVM, normalised, $+1.9$ ($p=0.039$). Best absolute accuracies (an untuned RBF SVM on all features): 81.4\% on CHB-MIT with per-patient normalised features, 76.8\% raw, and 79.2\% on Siena. Under univariate top-$k$ selection every model selected all 36 amplitude features first and the ``chirplet + amplitude'' result was in fact amplitude-only, which is why the selection regime was replaced. \qact\ versus classical features, all features and regularisation: logistic L2 62.7 vs 63.6\%, L1 62.9 vs 63.4\%, RBF SVM 66.4 vs 67.5\%; sampled versus argmax selection for the SVM $-1.93$ points, $p=0.0032$. Siena (14 patients, 45 seizures, never inspected during development): amplitude only 69.5 / 65.9\% (logistic / SVM), chirplet + amplitude 78.9 / 79.2\%, \qact\ + amplitude 78.5 / 79.1\%; chirplet gain $+9.43$ ($p=0.0047$) and $+13.31$ ($p=0.0040$); \qact\ versus classical $+0.30$ and $-1.99$, not significant.

\paragraph{Atom quality versus feature value.} Atom-level improvements that are real (the refiner, cubic and skew atoms, measurement as a proposal distribution that halves the error on planted chirplets) produced no downstream gain. The best of them had the best atoms and the worst features, and on real EEG its reconstruction advantage did not exist (0.3797 versus 0.3828), because 1/f EEG has no clean atoms to find.
