...
classical + 0.5\,Hz grid + coordinate refiner & \denoiseArmAScore & & & vs \qact\ parity hw (\denoiseRefQ): better on \denoiseArmAVsQBetter/\denoiseArmAVsQN, $p=\denoiseArmAVsQP$ \\
classical + 0.5\,Hz grid, seed grid to 78\,Hz & \denoiseArmBScore & & & vs \qact\ parity hw: better on \denoiseArmBVsQBetter/\denoiseArmBVsQN, $p=\denoiseArmBVsQP$ \\
\fi
\midrule
...
\section{Conclusion}
A quantum adaptive chirplet transform can be built exactly, refined exactly, brought to full functional parity with a strong classical engine, and run for the shortest atoms on a quantum processor. Under pre-registered comparisons with paired tests on real EEG it is its classical peer's equal and beats nothing; each apparent advantage was a dictionary or refiner choice the classical engine can copy, and two of the controls that show it are new here. The exact dequantisation and the coherent residual make precise what is and is not quantum about the construction. A fully controlled parity, with the controls named before the data were seen, is the baseline a claimed quantum advantage for recorded signals has to clear.
