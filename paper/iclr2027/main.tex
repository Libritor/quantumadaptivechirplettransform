...
% Submission is double blind. Author block for the camera-ready:
% Khalil Chaghouri, Alexander Vicol, Steve Mann -- University of Toronto.
...
(2) A pre-registered audit on real EEG (\S\ref{sec:exp}): denoising with ground truth on 36 subjects, seizure detection on 37 patients of two databases, per-person forecasting on 983 hours, with the three controls that each dissolved an apparent advantage, two of them run here for the first time.
...
The captured energy $E(\theta)=|\braket{\psi_\theta}{r}|^2/\norm{\psi_\theta}^2$ depends on every phase-gate angle $a$ through a factor $e^{ia}$ multiplying part of the overlap, so $E=A+B\cos a+C\sin a$ and $\partial E/\partial a=[E(a+\pi/2)-E(a-\pi/2)]/2$ exactly \citep{mitarai2018circuit,schuld2019evaluating}. The chain rule maps the $n+n(n-1)/2$ angles onto $f$ and $c$ with the weights of eq.~\eqref{eq:twolocal}; the envelope parameters are not gate angles and use central differences, as a device would. Closed-form shifted evaluations agree with genuinely shifted circuits to $2.8\times10^{-9}$. Four steps cost 112 circuit evaluations per atom; on planted signals they recover a 6\,Hz burst at 6.01\,Hz from a 6.50\,Hz seed and reduce reconstruction error by about 25\%. A cheaper rule, used on hardware, is a three-point coordinate search (two evaluations per coordinate, keep the best of three); it costs 8 evaluations per step and turns out to be \emph{better} than the gradient (\S\ref{sec:control}), which is the second of the effects we later have to control for.
...
Three things follow. First, at equal refinement rule the classical engine is better than \qact: with the same coordinate search the classical engine is \fairCoordVsHwRelTwelve\% below \qact\ at order 12, better on \fairCoordVsHwBetterTwelve\ of \fairN\ windows ($p=\fairCoordVsHwPTwelve$); the coordinate search itself costs the classical engine only \fairClCoordRelTwelve\% against sixty Adam steps at a tenth of the time, so it is a good classical refiner, which is why it looked good on \qact. Second, the 0.5\,Hz grid changes nothing on this metric at these orders (\fairClFineAdamRelTwelve\% at order 12, $p=\fairClFineAdamPTwelve$); the denoising gain it produced in \S\ref{sec:denoise} is a property of the artifact rule's frequency thresholds, not of reconstruction. Third, \qact\ at parity is \fairQParityRelTwelve\% \emph{better} than the reference in the mean at order 12 but better on only \fairQParityBetterTwelve\ of \fairN\ windows, with a median difference of $\fairParityMedianDiffTwelve$: the mean is carried by \fairRefFailTwelve\ windows on which the classical engine leaves more than 90\% of the energy. Those windows are dominated by 60\,Hz mains interference (\fairDriftFailTwelve\% of their energy above 45\,Hz, against \fairDriftOKTwelve\% for the rest), which the classical seed grid, stopping at 40\,Hz with an in-band refiner, cannot represent, while \qact's exact frequency update reads the periodogram peak wherever it is and its refiner is not band-restricted. Extending the classical seed grid to 78\,Hz leaves \fairLowFailTwelve\ such windows, and the classical engine is then \fairLowVsParityRelTwelve\% relative to \qact\ parity, better on \fairLowVsParityBetterTwelve\ of \fairN\ ($p=\fairLowVsParityPTwelve$); with the backfit pass \qact\ has, it is \fairLowBfVsParityRelTwelve\% (better on \fairLowBfVsParityBetterTwelve, $p=\fairLowBfVsParityPTwelve$). Every remaining difference is a band, dictionary or refiner choice the classical engine can make.

\subsection{Downstream: does the representation change what is learned?}
\label{sec:downstream}
The features are 13 energy-weighted aggregates per channel (band fractions, oscillatory versus transient split, spectral centroid and spread, mean absolute and signed sweep, duration, reconstruction error), invariant to the greedy order, plus per-channel log-RMS and log line length \citep{esteller2001line} computed before normalisation. Table~\ref{tab:downstream} summarises the pre-registered comparisons; full tables are in Appendix~\ref{app:downstream}.
...
seizure, CHB-MIT & \qact\ vs classical features, all features + regularisation & $-0.5$ to $-3.1$ pts & classical better, one $p=0.003$ \\
seizure, Siena & \qact\ vs classical features & $+0.3$, $-2.0$ pts & indistinguishable \\
...
\paragraph{Real device.} We executed the windowed construction on \texttt{\qpuBackend}, a 156-qubit Heron processor, for \qpuN\ circuits: six public EEG windows (three subjects, two channels) at two short widths, each in the semiclassical (dynamic) form and in a unitary-QFT form that admits gate twirling, plus one long-atom case in both forms, \qpuShots\ shots each with dynamical decoupling (jobs \texttt{\qpuJobs}, \qpuSeconds\ s of processor time). The classical certificate for every circuit is the exact distribution of Proposition~\ref{prop:dequant}. For the \qpuShortN\ short-atom circuits the device selected the exact in-band atom in \qpuShortPeakOK\ cases, with mean selected-atom energy \qpuShortSel\ (minimum \qpuShortSelMin) and mean Hellinger fidelity \qpuShortFid\ against \qpuShortFidSim\ predicted by the fake-backend noise model; the dynamic and unitary forms gave \qpuDynSel\ and \qpuUniSel\ (\qpuDynCZ\ and \qpuUniCZ\ CZ). The long-atom circuits (\qpuLongCZ\ CZ) reached fidelity \qpuLongFid\ and selected-atom energy \qpuLongSel, i.e. selection is lost, as the noise model predicted. Fig.~\ref{fig:device} shows one device histogram against the exact peak and the predicted-versus-measured selection quality for all circuits.

\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{fig_device.pdf}
\caption{Left: device histogram of a 6-qubit windowed selection circuit on \texttt{\qpuBackend} with the exact in-band peak marked. Right: selected-atom energy predicted by the calibrated noise model against the value measured on the device, all circuits.}
\label{fig:device}
\end{figure}

\paragraph{Speed.} Table~\ref{tab:speed} prices the same job (four EEGMAT windows, six atoms, OMP) on every engine. The circuits work: run shot by shot in a simulator with the coarse window grids, they reach the exact simulation's error (\speedQAerErr\ versus \speedQSimErr). On a Heron processor the optimised circuits would take \speedHeron\ per window, and three published techniques (zero repetition delay, multi-programming \speedPack\ circuits per chip \citep{niu2023multiprogramming,ohkura2022simultaneous}, and adaptive shot allocation by successive elimination \citep{huang2025gambling}, which we ran and which cut shots \speedAdaptiveShots$\times$ at identical error) bring that to \speedAllThree\ --- still about 140$\times$ slower than the classical engine on a CPU and 12,000$\times$ slower than the batched GPU engine. Per shot, the 250\,$\mu$s repetition delay dwarfs the 16--96\,$\mu$s circuit, and each circuit needs thousands of shots to estimate a distribution an FFT computes exactly in microseconds.
...
\section{Discussion}
\paragraph{What survives.} Three things are genuinely quantum-native in \qact\ and none is an advantage. The frequency axis is free, which saves a constant factor the classical engine buys with a larger grid. The dictionary is gate-native, which removes half of the QRAM assumption in quantum matching pursuit; the other half, the signal, remains, and Proposition~\ref{prop:coherent} shows the residual can be kept coherent at the price of a success probability that equals the unexplained energy. Short atoms run on today's hardware with the right selection, which is a statement about circuit design under noise, not about speed. Everything else that looked like a win had a classical explanation, found by a control, and the sequence is the useful part of the paper: a strong-looking result on real data (17 of 18 subjects, $p<10^{-4}$) was wrong for a reason only the third control revealed, and the pattern recurred for the refiner and the seed band. \citet{bowles2024better} and \citet{schuld2022quantum} argue that quantum learning needs matched baselines and a goal other than advantage; a representation is where the baseline is easiest to match exactly, because the classical peer is a program, not a model class.
...
\paragraph{Limitations.} The audit covers one transform family, three EEG databases and one device; the hardware runs demonstrate selection for individual circuits, not a full decomposition on a QPU; the coherent-residual construction is verified at $n=6$ and its Gaussian preparation is not optimised; and the forecasting and classification models are those a practitioner would try, not the strongest possible quantum architectures. Nothing here rules out an advantage for a quantum-native signal.

\section{Conclusion}
A quantum adaptive chirplet transform can be built exactly, refined exactly, brought to full functional parity with a strong classical engine, and run for short atoms on a quantum processor. Under pre-registered comparisons with paired tests on real EEG it is its classical peer's equal and beats nothing; each apparent advantage was a dictionary or refiner choice the classical engine can copy, and two of the controls that show it are new here. The exact dequantisation and the coherent residual make precise what is and is not quantum about the construction. A fully controlled parity, with the controls named before the data were seen, is the baseline a claimed quantum advantage for recorded signals has to clear.
